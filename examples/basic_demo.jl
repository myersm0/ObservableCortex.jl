...
# you can then add arbitrary additional elements, like a colorbar:
cbar = Colorbar(
	fig, 
	bbox = BBox(480, 720, 400, 360), 
	limits = colorrange, 
	colormap = coolhot, 
	vertical = false, 
	width = 240, 
...
initialize_adjacencies!(px.surface) # needed for finding borders below
...
for k in ks
	...
	meshscatter!(ax, coords'; color = :yellow, markersize = 0.65)
end
...
n_timepoints = 500
n_vertices = size(c, Exclusive())

# define some faux time series data for demonstration
time_series = zeros(n_vertices, n_timepoints)
Threads.@threads for t in 1:n_timepoints
	phase = 2π * t / n_timepoints
	for v in 1:n_vertices
		x = (v / n_vertices) * 4π
		time_series[v, t] = sin(x - phase)
	end
end

current_frame = Observable(1)
current_data = @lift time_series[:, $current_frame]

plot!(montage, current_data; colormap = coolhot, colorrange = (-1, 1))

control_grid = GridLayout(fig[3, 1:2])

slider = Slider(control_grid[1, 1:3], range = 1:n_timepoints, startvalue = 1)
connect!(current_frame, slider.value)
Label(control_grid[1, 4], @lift("Frame: $($current_frame)/$n_timepoints"))
